...
func handleBind(w ldap.ResponseWriter, m *ldap.Message) {
	...
	if r.AuthenticationChoice() == "simple" {
		var password = fmt.Sprintf("%s", r.Authentication())
		println(password)
		if string(r.Name()) == "login" && password == "password" {
			w.Write(res)
			return
		}
...
func handleSearch(w ldap.ResponseWriter, m *ldap.Message) {
	...
	var x = fmt.Sprintf("%s", r.FilterString())
	println(x)
	var infoFromFilter = strings.Split(x, "=")
	var username = infoFromFilter[3][0:(len(infoFromFilter[3]) - 2)]
	var password = infoFromFilter[2][0:(len(infoFromFilter[2]) - 5)]

	if r.Filter() != nil && username == "amariwan" && password == "dontBeDone" {
		e := ldap.NewSearchResultEntry(r.FilterString())
		e.AddAttribute("mail", "dev@aland-mariwan.de")
		e.AddAttribute("company", "tasio")
		e.AddAttribute("department", "DSI/SEC")
		e.AddAttribute("l", "tasio")
		e.AddAttribute("mobile", "01727134842")
		e.AddAttribute("telephoneNumber", "01727134842")
		e.AddAttribute("cn", "Aland Mariwan")

		w.Write(e)
	} else if r.Filter() != nil && username == "admin" && password == "admin" {
		e := ldap.NewSearchResultEntry(r.FilterString())
		e.AddAttribute("mail", "dev@aland-mariwan.de")
		e.AddAttribute("company", "tasio")
		e.AddAttribute("department", "DSI/SEC")
...
8i1UQF6AzwIDAQABo08wTTAOBgNVHQ8BAf8EBAMCACQwDQYDVR0OBAYEBAECAwQwt
